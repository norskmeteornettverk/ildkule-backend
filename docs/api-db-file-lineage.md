...
| Event | same | `analysis.orbit.perihelion_distance_au`, `eccentricity`, `inclination_deg`, `ascending_node_deg`, `argument_of_perihelion_deg`, `mean_anomaly_deg`, `epoch` | observasjonsdrevet solve fra `observation_trail_point.event_timestamp_us` (eksakt Unix-mikrosekunder; `event_timestamp` i API er en avledet sekundverdi), `centroid2_coord_*`, `centroid_coord_*`, `ams_coord_*`, `coord_*`, observasjonssted fra `observation_cam_data.summary_*`, fallback `event.track_speed`, `event.radiant_ecl_long`, `event.radiant_ecl_lat`, fallback `event.radiant_ra`, `event.radiant_dec`, `event.date` | `event.txt`, `*.stat`, `*.res`, event folder | `[trail]` timestamps, `coordinates`, `ams_coords`, `centroid`, `centroid2`, `[summary]` latitude/longitude/elevation, fallback `speed`, `ecl_long`, `ecl_lat`, `ra/dec`, UTC event time | krysspeilede hendelser prøver først å regne banen fra målte spor i trail-data. `centroid2` brukes nå som en faktisk punktkilde når den finnes, og solve-linjen bestemmer retningen fra observasjonene selv i stedet for `track_start/end`. AMS-koordinater brukes per frame når de finnes. | derived | `fastapi_app/app/utils/orbit_solver.py`, `fastapi_app/app/utils/serialization.py`, `scripts/validate_orbit_against_tables.py` |
