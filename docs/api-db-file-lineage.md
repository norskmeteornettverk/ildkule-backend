...
| Event | same | `analysis.orbit.perihelion_distance_au`, `eccentricity`, `inclination_deg`, `ascending_node_deg`, `argument_of_perihelion_deg`, `mean_anomaly_deg`, `epoch` | observasjonsdrevet solve fra `observation_trail_point.event_timestamp`, `coord_*`, `ams_coord_*`, observasjonssted fra `observation_cam_data.summary_*`, fallback `event.track_speed`, `event.radiant_ecl_long`, `event.radiant_ecl_lat`, fallback `event.radiant_ra`, `event.radiant_dec`, `event.date` | `event.txt`, `*.stat`, `*.res`, event folder | `[trail]` timestamps, `coordinates`, `ams_coords`, `[summary]` latitude/longitude/elevation, fallback `speed`, `ecl_long`, `ecl_lat`, `ra/dec`, UTC event time | krysspeilede hendelser prøver først å regne banen fra målte spor i trail-data. AMS-koordinater brukes per frame når de finnes. `centroid.txt` og `centroid2.txt` lagres nå også på observasjonen og trail-punktene, men brukes ikke i orbit-beregningen i denne første slicen. | derived | `fastapi_app/app/utils/orbit_solver.py`, `fastapi_app/app/utils/serialization.py`, `scripts/validate_orbit_against_tables.py` |
...
| Trail points | `/api/observations/{id}/trail` | `has_ams_coords`, `has_centroid`, `has_centroid2`, `trailPoints[].frame_index`, `pixel_x`, `pixel_y`, `event_timestamp`, `coord_long`, `coord_lat`, `ams_coord_long`, `ams_coord_lat`, `centroid_coord_long`, `centroid_coord_lat`, `centroid2_coord_long`, `centroid2_coord_lat` | `observation_trail_point.*`, `observation_cam_data.trail_ams_coords`, `observation_cam_data.trail_centroid`, `observation_cam_data.trail_centroid2` | `event.txt`, `centroid.txt`, `centroid2.txt` | `[trail]` arrays fra `event.txt`, radfiler fra centroid-filene | normaliserer standardkoordinater, AMS-koordinater og nå også centroid-koordinater per frame. Centroid-rader matches først på UTC-timestamp, ellers på radrekkefølge hvis lengdene er like. | direct/derived | `fastapi_app/app/services/file_mapper.py`, `fastapi_app/app/models/observation_trail_point.py`, `fastapi_app/app/services/event_service.py`, `fastapi_app/app/schemas/event.py` |
...
| Insight | `/api/insights/coordinates` (kanonisk), skjulte alias `/api/insight/coordinates`, `/api/insight/{report_name}` og `/api/report/coordinates` | `id`, `datetimetag`, `station_cam`, `number_of_stations`, `lat`, `lng`, `slat`, `slng`, `radiant_ra`, `radiant_dec`, `radiant_ecl_lat`, `radiant_ecl_long`, `track_speed`, `track_endheight`, `radiant_shower`, `date`, `triangulation`, `proper_triangulation`, `ai_score` | miks av `event.*`, `observation_cam_data.summary_meteor_probability`, `station.station_name`, `cam.cam_name` | `*.res`, `*.stat`, `event.txt`, station folder, cam folder | løst banegrunnlag + solved event summary + observasjonssammendrag | koordinatrapporten bygges nå i Python fra event og observasjoner, ikke lenger bare som `lat/lng`-uttrekk. Kanonisk rute støtter nå `from_date`, `to_date`, `stations`, `cross_station_confirmed`, `candidate` og `includeDeleted`. De skjulte aliasene speiler også legacy-feltene `StationCam`, `NumberOfStations`, `ProperTriangulation`, `SourceBadDetection` og `MeteorScoreHighest` for inspiserte frontend-grener. | derived | `fastapi_app/app/services/event_service.py`, `fastapi_app/app/routers/events.py` |
...
| `station/cam/event.txt` key `trail:ams_coords` | observation | yes | yes | Alternativ AMS-sporserie som finnes i enkelte `event.txt`-filer med `ams = 1`; parseren leser den nå inn som rå `trail_ams_coords` og normaliserer den videre til `ams_coord_long` og `ams_coord_lat` per trail frame | `fastapi_app/app/services/file_mapper.py:100`, `fastapi_app/app/models/observation_cam_data.py:52`, `fastapi_app/app/models/observation_trail_point.py:37` |
| `station/cam/centroid.txt` | observation | yes | yes | Rå centroid-rader leses inn som `trail_centroid` og normaliseres videre til `centroid_coord_long` og `centroid_coord_lat` per trail frame når radene kan matches | `fastapi_app/app/services/file_mapper.py`, `fastapi_app/app/models/observation_cam_data.py`, `fastapi_app/app/models/observation_trail_point.py` |
| `station/cam/centroid2.txt` | observation | yes | yes | Alternativ centroid-radfil leses inn som `trail_centroid2` og normaliseres videre til `centroid2_coord_long` og `centroid2_coord_lat` per trail frame når radene kan matches | `fastapi_app/app/services/file_mapper.py`, `fastapi_app/app/models/observation_cam_data.py`, `fastapi_app/app/models/observation_trail_point.py` |
