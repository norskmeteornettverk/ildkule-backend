...
| Tutorial | `GET /api/tutorial` | `title`, `intro`, `sections[].id`, `title`, `bullets[]` | - | - | statisk backend-innhold, na justert mot seksjonene i frontend-modalens tutorial | egen innholdsflate for tutorial, atskilt fra statusflagget paa bruker; frontend-modalens bilder, videolenker og publiseringsfot er fortsatt ikke egne API-felter | api-only | `fastapi_app/app/routers/users.py`, `frontend-branch/ildkule-frontend/src/components/UserTutorialModal.vue` |
