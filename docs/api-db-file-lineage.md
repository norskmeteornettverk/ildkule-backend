...
| Event | same | `analysis.orbit.perihelion_distance_au`, `eccentricity`, `inclination_deg`, `ascending_node_deg`, `argument_of_perihelion_deg`, `mean_anomaly_deg`, `epoch` | observasjonsdrevet solve fra `observation_trail_point.event_timestamp`, `coord_*`, `ams_coord_*`, observasjonssted fra `observation_cam_data.summary_*`, fallback `event.track_speed`, `event.radiant_ecl_long`, `event.radiant_ecl_lat`, fallback `event.radiant_ra`, `event.radiant_dec`, `event.date` | `event.txt`, `*.stat`, `*.res`, event folder | `[trail] timestamps`, `coordinates`, `ams_coords`, `[summary] latitude/longitude/elevation`, fallback `speed`, `ecl_long`, `ecl_lat`, `ra/dec`, UTC event time | krysspeilede hendelser prøver først observasjonsdrevet heliosentrisk solve fra trail-data med AMS-koordinater per kamera når de finnes. Runtime beholder bare denne solve-linjen når trail-fitten holder seg stabil nok (medianresidual <= 0.3 km, maksresidual <= 0.8 km). Hvis geometrien er stabil men `mean_anomaly_deg` fortsatt driver for langt fra fallback-linjen, beholder runtime de observerte baneelementene men gjenbruker fallback `mean_anomaly_deg` og `epoch`. Ellers brukes hele stat-beregningen. Elliptiske løsninger normaliserer `mean_anomaly_deg` til `0..360`. | derived | `fastapi_app/app/utils/orbit_solver.py`, `fastapi_app/app/utils/serialization.py` |
...
| Observation | event detail | `observations[].trail_frames`, `trail_duration`, `trail_slope`, `trail_offset`, `trail_speed`, `trail_correlation`, `trail_positions`, `trail_timestamps`, `trail_coordinates`, `trail_ams_coords`, `trail_gnomonic`, `trail_midpoint`, `trail_arc`, `trail_brightness`, `trail_dct_midpoint`, `trail_dct`, `trail_size`, `trail_frame_brightness`, `has_ams_coords` | `observation_cam_data.trail_*` | `event.txt` | `[trail]` section | `event.txt` er kameranivaaets deteksjons- og målefil; `trail:ams_coords` leses nå inn når den finnes, og `has_ams_coords` avledes fra om observasjonen faktisk har en slik råserie | direct/derived | `fastapi_app/app/services/file_mapper.py:81`, `fastapi_app/app/models/observation_cam_data.py:44`, `fastapi_app/app/utils/serialization.py:894` |
...
| Trail points | `/api/observations/{id}/trail` | `has_ams_coords`, `trailPoints[].frame_index`, `pixel_x`, `pixel_y`, `event_timestamp`, `coord_long`, `coord_lat`, `ams_coord_long`, `ams_coord_lat` | `observation_trail_point.*`, `observation_cam_data.trail_ams_coords` | `event.txt` | `[trail]` arrays normalisert per frame | normaliserer nå både standardkoordinater og AMS-koordinater per frame; toppnivåfeltet `has_ams_coords` avledes fra om observasjonen har en rå AMS-serie | direct/derived | `fastapi_app/app/services/file_mapper.py:487`, `fastapi_app/app/models/observation_trail_point.py:27`, `fastapi_app/app/services/event_service.py:507`, `fastapi_app/app/schemas/event.py:641` |
