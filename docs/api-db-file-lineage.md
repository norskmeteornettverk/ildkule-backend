...
| Event list filters | `/api/events` query | `searchTerm`, `stationName`, `year`, `eventType`, `from_date`, `to_date`, `includeDeleted`, `page`, `limit`, `orderby`, `order` | hovedsakelig `event.*`, pluss `station.station_name` via `cam` og `observation_cam_data` | event folder, station folder | tekstsoek i `location`/`datetimetag`, aar fra `event.date`, datointervall mot `event.date`, stasjonsfilter via observasjonsjoin | alle oppgitte filtre brukes naa i samme query; `from_date`/`to_date` er inklusiv `YYYY-MM-DD`; paginering og sortering gjelder ogsaa for filtrerte treff; bootstrap-skjemaene har egne `event.date`-indekser for denne lesestien | api-only | `fastapi_app/app/routers/events.py`, `fastapi_app/app/services/event_service.py`, `database/build_db.sql`, `database/build_db_postgres.sql` |
